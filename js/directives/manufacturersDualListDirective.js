define(['controllers/controllerModule', 'directives/directiveModule', 'jquery', 'underscore'], function(controllerModule, directiveModule, $, underscore) {
    directiveModule.directive('manufacturerDualList', ["$filter", "$timeout", "sharedService", function($filter, $timeout, sharedService) {
        var diffArray = [];
        return {
            restrict: 'E',
            scope: {
                ...
                moveAllBtn: '=moveAllBtn'
            },
            templateUrl: 'views/manufacturersDualList.html',
            link: function(scope, elem, attrs) {
                ...
                //fix for manufacturers filter box to display 3rd character and start search
                ...
                scope.removeFromRight = function(item, from, to) {
                    if (item !== undefined) {
                        ...
                        sharedService.setSelectedManufacturers(from);
                    }
                    if ($(".selected").children().length === 0) {
                ...
                scope.copyToRight = function(item, from, to) {
                    if (item !== undefined) {
                        ...
                        sharedService.setSelectedManufacturers(to);
                        $(".unselected option:selected").removeAttr("selected");
